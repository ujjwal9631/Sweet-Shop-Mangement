...
class ApiService {
  ...
  async getAllSweets(page = 1, limit = 12): Promise<SweetsResponse> {
    const response = await this.api.get<SweetsResponse>('/sweets', { params: { page, limit } });
    return response.data;
  }
  ...
  async searchSweets(params: SearchParams): Promise<SweetsResponse> {
    const response = await this.api.get<SweetsResponse>('/sweets/search', { params });
    return response.data;
  }
  ...
  async updateSweet(id: string, data: UpdateSweetData): Promise<SweetResponse> {
  ...
  // Inventory endpoints
  async purchaseSweet(id: string, quantity = 1): Promise<PurchaseResponse> {
    const response = await this.api.post<PurchaseResponse>(`/sweets/${id}/purchase`, { quantity });
    return response.data;
  }

  async restockSweet(id: string, quantity: number): Promise<RestockResponse> {
    const response = await this.api.post<RestockResponse>(`/sweets/${id}/restock`, { quantity });
    return response.data;
  }
}
